...
export function Navigation({ activeTab, onTabChange }: NavigationProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-solarized-base3 border-t border-solarized-base2 px-2 pt-2 pb-3 z-50">
        ...
        <div className="flex gap-1 min-w-max px-2 h-full">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex flex-col items-center py-1 px-3 rounded-lg transition-all duration-200 ease-in-out min-w-0 flex-shrink-0 ${
                activeTab === id
